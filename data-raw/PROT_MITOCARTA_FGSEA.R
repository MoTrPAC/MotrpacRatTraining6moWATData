...
mito_df <- file.path("data-raw", "Human.MitoCarta3.0.xls") %>%
...
# List of 64 pathways to test
MITOCARTA_PATHWAYS <- mito_df %>%
...
PROT_MITOCARTA_FGSEA <- map(PROT_DA, function(res_i) {
  ...
  map(names(stats), function(contrast_i) {
    ...
    left_join(dplyr::select(mito_df, pathway, hierarchy), by = "pathway") %>%
