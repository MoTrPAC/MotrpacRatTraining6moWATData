...
## Metabolomics (quick) --------------------------------------------------------
METAB_WGCNA <- run_WGCNA(eset = METAB_MSNSET,
                         power = 12,
                         module_prefix = "M",
                         merge_modules = FALSE)
...
PROT_WGCNA <- run_WGCNA(eset = PROT_MSNSET,
                        power = 12,
                        module_prefix = "P",
                        merge_modules = TRUE)
...
## Transcriptomics (incredibly slow) -------------------------------------------
...
TRNSCRPT_WGCNA <- run_WGCNA(eset = TRNSCRPT_MSNSET,
...
map2(.x = all_WGCNA, .y = paths, .f = writexl::write_xlsx, .progress = TRUE)
