...
Observation 6 is outlying in the QQ-plot. Simplifying the model may fix this.

```{r}
anova(fit.glucagon, test = "F")
```

Only sex is a significant predictor.

```{r}
fit.glucagon.1 <- update(fit.glucagon, formula = . ~ sex)
anova(fit.glucagon.1, fit.glucagon, test = "F")
```
...
Simplifying the model did not fix the problem with observation 6, which has a much lower glycogen concentration than the other samples in the 2W female group. Unfortunately, there is no biological justification for removal, so we will keep it.

There is no significant difference between the models at the 0.05 level, but we will lose the ability to compare trained groups to their sedentary counterparts, so we will use the interaction model.
